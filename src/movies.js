...
function howManyMovies(moviesArray) {
    return moviesArray.filter(movie=> movie.director==="Steven Spielberg" && movie.genre.indexOf("Drama")>=0).length   
...
function scoresAverage(moviesArray) {
    if (moviesArray.length===0){ return 0}
      ...
      return Math.round((moviesArray.reduce((acumulador, pelicula) => {
        return acumulador + pelicula.score}, 0))/(moviesArray.length)*100)/100
      }
  }
...
function dramaMoviesScore(moviesArray) {
    if (moviesArray.filter(movie=> movie.genre.indexOf("Drama")>=0).length!=0){
    return Math.round((moviesArray.filter(movie=>movie.genre.indexOf("Drama")>=0).reduce((acumulador, pelicula) => {
        return acumulador + pelicula.score}, 0))/(moviesArray.filter(movie=> movie.genre.indexOf("Drama")>=0).length)*100)/100
    }else{
        return 0
    }    
}
